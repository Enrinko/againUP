...
class SiteController extends AbstractController
{
    ...
    public function __construct(EntityManagerInterface $manager, private FormLoginAuthenticator $authenticator)
    {
    ...
    public function showLecture(EntityManagerInterface $manager,Breadcrumbs $breadcrumbs, int $id): Response
    {
        $links = ['Главная' => "/", 'Лекции' => "/lecture/list", 'Лекция' => ""];
        $this->createBreadcrumb($links, $breadcrumbs);
        $array = ['this' => 'Лекция', 'lecture' => $manager->getRepository(Lectures::class)->find($id)];
    ...
    public function about(Breadcrumbs $breadcrumbs): Response
    {
        $links = ['Главная' => "/", 'О нас' => ""];
        $this->createBreadcrumb($links, $breadcrumbs);
        $array = ['tests' => $this->manager->getRepository(TestsOfUser::class)->count(['User' => $this->getUser()])
            . '/' .
            $this->manager->getRepository(Tests::class)->count(['id' => '?']), 'this' => 'О нас',];
    ...
    public function userCab(IdentifyService $service, Breadcrumbs $breadcrumbs): Response
    {
        ...
        $this->createBreadcrumb($links, $breadcrumbs);
        $array = ['username' => $this->getUser()->getUserIdentifier(), 'role' => $service->identifyRole(), 'course' => $this->isGranted('ROLE_PREMIUM') ? 'TechLand Plus' : 'TechLand Base', 'tests' => $this->manager->getRepository(TestsOfUser::class)->count(['User' => $this->getUser()]) . '/' . $this->manager->getRepository(Tests::class)->count(['id' => '?']), 'this' => 'Личный кабинет',];
    ...
    ): Response
    {
        ...
        $array = ['form' => $form->createView(), 'title' => 'Зарегистрируйтесь', 'text' => 'Уже есть аккаунт? Войдите!', 'toWhere' => 'login'];
    ...
    public function login(Request $request): Response
    {
        ...
        $array = ['form' => $form->createView(), 'title' => 'Войти', 'text' => 'Нет аккаунта? Зарегистрируйтесь!', 'toWhere' => 'reg', 'method' => 'POST'];
        return $this->render('auth.html.twig', $array);
    }
